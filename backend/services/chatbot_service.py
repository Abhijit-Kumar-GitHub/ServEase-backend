...
def remove_markdown(text):
    """Removes basic Markdown formatting (bold, italic) from a string."""
    text = re.sub(r'\*', '', text)  # Remove asterisks
    text = re.sub(r'\_', '', text) #Remove underscores
    text = re.sub(r'\`', '', text) #Remove code quotes
    return text
...
class GeminiService:
    ...
    def __init__(self, model_name='gemini-pro'):
        ...
        self.prompt_prefix = (
            ...
            "1. **Always prioritize safety:** Emphasize safety precautions and warnings before explaining any repair or maintenance procedure. If a task is inherently dangerous, complex, or requires specialized skills, advise the user to consult a qualified professional. Briefly mention that our platform connects users with vetted and experienced professionals in Jalandhar.\n"
            ...
            "   - **Crucially, your responses MUST be plain text only. Do NOT use any Markdown formatting, including bold text, italic text, code blocks, headers, or any other formatting elements.** Use only whitespace and newlines to structure your responses. Your output is being directly displayed as plain text, and any formatting codes will be visible to the user and look unprofessional. \n"
    ...
    def generate_streaming_response(self, prompt):
        ...
        try:
            for chunk in self.model.generate_content(full_prompt, stream=True):
                yield remove_markdown(chunk.text) # post process the output after getting a chunk
        except Exception as e:
            print(f"Gemini API Error: {e}")
